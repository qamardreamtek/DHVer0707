...
{
    // 同步,
    // from PROG_WRT to AspNetUserRoles



    public class DhReportService
    {
        ...
        public string GenerateExcel(IQueryable query, string catName, string sheetName)
        {
            var columns = GetProperties(query.ElementType);
            var stream = new MemoryStream();
            var sFile = "StockMovementPalletReport_" + GetDteFrmCtl(DateTime.Now) + ".xlsx";


            string toDownload = $@"{catName}/{sFile}";

            string file2 =$@"wwwroot/{toDownload}";

            using (var document = SpreadsheetDocument.Create(file2, SpreadsheetDocumentType.Workbook))
            {
                var workbookPart = document.AddWorkbookPart();
                workbookPart.Workbook = new Workbook();
