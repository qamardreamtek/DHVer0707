...
namespace RadzenDh5
{
    public partial class SecurityService
    {
        ...
        //public DhReportService(Mark10Sqlexpress04Context context, IWebHostEnvironment WebHostEnvironment)
        //{
        ...
        public async Task<ApplicationUser> CreateUser(ApplicationUser user)
        {
            ...
            // NOTE by Mark, 04/26
            await EnsureDhSucceeded(result, user);
        ...
        public async Task<ApplicationUser> GetUserByEmail(string str)
        {
            var user = await userManager.FindByEmailAsync(str);
        ...
        public async Task<ApplicationUser> GetUserByName(string str)
        {
            var user = await userManager.FindByNameAsync(str);
        ...
        // NOTE by Mark, 04/26
        // 密碼長度要求最少 8碼, 如有不合的 usermst 進來, 按之前的做法,  throw new ApplicationException(message);
        // 要改為寫出 異常, 讓 App 繼續運行
        private async Task EnsureDhSucceeded(IdentityResult result, ApplicationUser user)
        {
            if (!result.Succeeded)
            {
                var message = string.Join(", ", result.Errors.Select(error => error.Description));
        ...
        public static async Task WriteThrow(string f1,string lines)
        {
   

            await File.WriteAllTextAsync(f1, lines);
        }
    }
}
